...
class GraphChiDatabase(baseFilename: String, origNumShards: Int) {
  ...
  val edgeIndexing : DatabaseIndexing = new DatabaseIndexing {
    ...
    def shardSize(idx: Long) = throw new UnsupportedOperationException
  ...
  var columns = scala.collection.mutable.Map[DatabaseIndexing, Seq[(String, Column[AnyRef])]](
    vertexIndexing -> Seq[(String, Column[AnyRef])](),
    edgeIndexing ->  Seq[(String, Column[AnyRef])]()
  )
  ...
  def createCategoricalColumn(name: String, values: IndexedSeq[String], indexing: DatabaseIndexing) = {
    ...
    columns(indexing) = columns(indexing) :+ (name, col.asInstanceOf[Column[AnyRef]])
    col
  }
  ...
  def createMySQLColumn(tableName: String, columnName: String, indexing: DatabaseIndexing) = {
    ...
    columns(indexing) = columns(indexing) :+ (tableName + "." + columnName, col.asInstanceOf[Column[AnyRef]])
    col
  }
